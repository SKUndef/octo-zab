    ...
    req({
        ...
        function(err, resp, body) {
            ...
            if (err === null) {
                ...
                authPendingTasks[server] = setTimeout(function() {
                    serverAuth(req, server);
                }, 10000);
            }
        }
    );
    ...
    req({
        ...
        function(err, resp, body) {
            ...
            if ((err === null) && (body.result !== undefined)) {
                ...
                log('err', 'request', server + ': trigger.get');
            }
        }
    );

    getIssuesTasks[server] = setTimeout(function() {
        getServerIssues(req, token, server);
    }, 180000);
}
...
io.on('connection', function(socket) {
    ...
    socket.on('servers.set', function(serversConf) {
        _.each(authPendingTasks, function(v, k) {
            clearTimeout(v);
        });
        _.each(getIssuesTasks, function(v, k) {
            clearTimeout(v);
        });
